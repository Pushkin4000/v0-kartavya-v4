  ...
  return (
    <Card className="overflow-hidden">
      <div className="relative h-48 overflow-hidden">
        <img src={getCategoryImage(foodItem.category)} alt={foodItem.name} className="w-full h-full object-cover" />
      ...
      <CardContent className="p-4">
        <div className="mb-2">
          ...
          <p className="text-sm text-gray-500 truncate">{foodItem.providerName || "Unknown Provider"}</p>
        </div>
        ...
        {foodItem.description && <p className="text-sm text-gray-600 mt-2 line-clamp-2">{foodItem.description}</p>}
      </CardContent>
      <CardFooter className="p-4 pt-0 flex justify-between">
        ...
        {isOwner && (
          <Button variant="outline" className="w-full" asChild>
            <Link to={`/my-listings/${foodItem.id}/edit`}>Edit Listing</Link>
          </Button>
        )}
        {!isNgo && !isOwner && (
          <Button variant="outline" className="w-full" asChild>
