import type { FoodItem, FoodCategory, FoodStatus } from "./types"
...
const mapFoodItemToSupabase = (item: Partial<FoodItem>) => {
  const formattedExpiryDate = item.expiryDate instanceof Date ? item.expiryDate.toISOString() : item.expiryDate
...
export const foodService = {
  getAllFoodItems: async (): Promise<FoodItem[]> => {
    try {
      ...
      console.log(`Retrieved ${data?.length || 0} food items from Supabase at ${timestamp}`)
  ...
  getFoodItemsByProvider: async (providerId: string): Promise<FoodItem[]> => {
    try {
      ...
      const { data, error } = await supabase.from("food_items").select("*").eq("provider_id", providerId)
  ...
  createFoodItem: async (item: Omit<FoodItem, "id" | "createdAt" | "status">): Promise<FoodItem> => {
    try {
      ...
      const { data, error } = await supabase.from("food_items").insert(itemToInsert).select("*").single()
  ...
  updateFoodItem: async (id: string, updates: Partial<FoodItem>): Promise<FoodItem | null> => {
    try {
      ...
      const { data, error } = await supabase.from("food_items").update(updateData).eq("id", id).select("*").single()
