  ...
  const getRecentOrders = () => {
    if (isProvider) {
      ...
      const providerOrderIds = new Set(
        mockOrders.filter((order) => order.items?.some((item) => item.providerId === user.id)).map((order) => order.id),
      )
  ...
  return (
    <PageLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {isProvider && (
          <>
            <Button className="h-auto py-6 flex flex-col items-center justify-center gap-2" asChild>
              <Link to="/new-listing">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardHeader>
            <CardTitle className="flex justify-between items-center">
              ...
              <Button variant="ghost" size="sm" asChild>
                <Link to={isProvider ? "/my-listings" : "/browse"}>View All</Link>
              </Button>
            </CardTitle>
            <CardDescription>
              {isProvider ? "Your recently added food listings" : "Food items available for pickup"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {recentFoodItems.length > 0 ? (
              ...
              <p className="text-gray-500 py-6 text-center">
                {isProvider ? "You haven't created any food listings yet." : "No food items are currently available."}
              </p>
            )}
          </CardContent>
          {recentFoodItems.length === 0 && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {isProvider ? "Orders received for your food items" : "Your recent pickup orders"}
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <li key={order.id} className="border-b pb-4 last:border-b-0 last:pb-0">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{isProvider ? `NGO: ${order.ngoName}` : ""}</p>
                      </div>
